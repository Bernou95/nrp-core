...
TEST(TestOpensimExecutable, TestOpensimn)
{	
	auto cfg = OpensimConfig(nlohmann::json());
	cfg.opensimRunPy() = TEST_OPENSIM_RUN_LOOP_FILE;
	...
	const std::string engineAddrArg = std::string("--") + OpensimConfig::EngineServerAddrArg.data();
	...
	{		
		...
		// Send init call
		RestClient::Response resp = RestClient::post(serverAddr + "/" + EngineJSONConfigConst::EngineServerInitializeRoute.data(), EngineJSONConfigConst::EngineServerContentType.data(), nlohmann::json({{OpensimConfig::ConfigType.m_data, cfg.writeConfig()}}).dump());
		...
		// Send runstep call
		resp = RestClient::post(serverAddr + "/" + EngineJSONConfigConst::EngineServerRunLoopStepRoute.data(), EngineJSONConfigConst::EngineServerContentType.data(), nlohmann::json({{OpensimConfig::EngineTimeStepName.data(), timeStep}}).dump());
		auto respParse = nlohmann::json::parse(resp.body);
		ASSERT_EQ(respParse[OpensimConfig::EngineTimeName.data()].get<float>(), timeStep);
	...
	else
	{
		try
		{
			// Child process, Opensim executable
			...
			// Start server
			auto &opensimExec = OpensimServerExecutable::resetInstance(argc, const_cast<char**>(argv.data()));

			ASSERT_EQ(opensimExec.serverRunning(), true);
			...
			// Wait for init call
			opensimExec.pyState().allowThreads();
			opensimExec.waitForInit();
			...
			ASSERT_EQ(opensimExec.serverRunning(), false);
			send = 4;
			ASSERT_EQ(commCtP.writeP(&send, 1, 5, 1), 1);
		}
		catch(std::exception&)
