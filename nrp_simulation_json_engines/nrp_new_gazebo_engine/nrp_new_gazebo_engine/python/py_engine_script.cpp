...
void PyEngineScript::registerDevice(std::string deviceName)
{
	...
	PtrTemplates<NewGazeboEngineDeviceController<PyObjectDevice>>::shared_ptr
	        newController(new NewGazeboEngineDeviceController<PyObjectDevice>(DeviceIdentifier(deviceName, "", PyObjectDevice::TypeName.data())));
...
void PyEngineScript::setNewGazeboServer(NewGazeboServer *pServer)
{
