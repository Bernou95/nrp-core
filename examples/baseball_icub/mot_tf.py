...
def robot_control(motors):
    r_shoulder = GazeboJointDataPack("icub_pid::r_shoulder_yaw", "gazebo")
    r_shoulder.data.position = 30. * motors.data[0]['V_m']

    global doInitialize
    if doInitialize:
        doInitialize = False
        r_elbow = GazeboJointDataPack("icub_pid::r_elbow", "gazebo")
        r_elbow.data.position = 2.
        eye_tilt = GazeboJointDataPack("icub_pid::eye_tilt", "gazebo")
