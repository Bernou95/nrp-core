    ...
    stages {
        ...
        stage('Build image') {
            ...
            steps {
                ...
                {
                    sh 'sudo apt install -y docker-compose && docker-compose build nrp-gazebo-nest-env && docker login -u $USER -p $PASSWORD ${NexusDockerRegistryUrl} && docker-compose push nrp-gazebo-nest-env'
                }
        ...
        stage('Build and test') {
            agent {
                docker {
                    ...
                    image "${NexusDockerRegistryUrl}/nrp-core/nrp-gazebo-nest-ubuntu20-env:latest"
