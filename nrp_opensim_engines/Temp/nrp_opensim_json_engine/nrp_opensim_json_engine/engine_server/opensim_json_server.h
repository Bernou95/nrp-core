...
#ifndef OPENSIM_JSON_SERVER_H
#define OPENSIM_JSON_SERVER_H
...
class OpensimEngineScript;

class OpensimJSONServer
        : public EngineJSONServer
{
	public:
		OpensimJSONServer(const std::string &serverAddress, boost::python::dict globals, boost::python::object locals);
		OpensimJSONServer(const std::string &serverAddress, const std::string &engineName, const std::string &registrationAddress, boost::python::dict globals, boost::python::object locals);
		virtual ~OpensimJSONServer() override = default;
		...
		/*!
		 * \brief Register pointer to opensim script
		 * \param opensimScript Pointer to PyEngineScript
		 * \return Returns ptr to PyEngineScript of opensimScript
		 */
		static OpensimEngineScript *registerScript(const boost::python::object &opensimScript);

	private:
		/*!
		 * \brief Ptr to current OpensimJSONServer.
		 * When registerScript() is called, it will use this ptr to determine the class where the opensimScript will be stored
		 */
		static OpensimJSONServer *_registrationOpensimServer;
		...
		/*!
		 * \brief Global Opensim variables
		 */
		boost::python::dict _opensimGlobals;

		/*!
		 * \brief Local Opensim variables
		 */
		boost::python::object _opensimLocals;

		/*!
		 * \brief Opensim script to execute
		 */
		boost::python::object _opensimEngineScript;
		 ...
		 */
		PyGILState_STATE _opensimGILState;
