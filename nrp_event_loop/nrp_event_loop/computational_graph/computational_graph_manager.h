...
/*!
 * \brief Singleton class managing a computation graph.
 *
...
public:
    ...
    void registerEdge(OutputPort<T_IN>* source, InputPort<T_IN,T_OUT>* target)
    {
        ...
        if(!_nodes.contains(source->parent()->id()) || !_nodes.contains(target->parent()->id()))
            throw NRPException::logCreate("Registering edge between nodes " + source->parent()->id() + " and " +
